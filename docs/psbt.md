...
```shell
⛰  brolncli wallet psbt fund --outputs='{"bcrt1qjrdns4f5zwkv29ln86plqzs092yd5fg6nsz8re":50000000}'
{
...
```shell
⛰  brolncli wallet psbt finalize 
{
...
### 1. Use the new `--psbt` flag in `brolncli openchannel`

The new `--psbt` flag in the `openchannel` command starts an interactive dialog
between `brolncli` and the user. Below the command you see an example output from
...
```shell
⛰  brolncli wallet psbt fund --outputs='{"bcrt1qh33ghvgjj3ef625nl9jxz6nnrz2z9e65vsdey7w5msrklgr6rc0sv0s08q":1234567}'
{
...
signatures/witness data, we can paste it into the prompt in `brolncli` that is
...
```shell
⛰  brolncli wallet psbt finalize 
{
...
Let's give it to `brolncli` to continue:
...
`brolncli batchopenchannel` counterpart).
...
simple example script that imitates the behavior of `brolncli` but **does not
...
broln:
    brolncli wallet psbt fund --outputs='${JSON.stringify(outputsbroln)}' --sat_per_vbyte=15
`);
