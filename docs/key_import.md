...
(`ListAccounts`, `ImportAccount`, `ImportPublicKey`) and the `brolncli wallet
accounts` command.

```shell
$ brolncli wallet accounts
NAME:
   brolncli wallet accounts - Interact with wallet accounts.

USAGE:
   brolncli wallet accounts command [command options] [arguments...]
...
`WalletKit.ListAccounts` RPC or `brolncli wallet accounts list` command can be used
to retrieve the details of accounts.

```shell
$ brolncli wallet accounts list
{
    "accounts": [
        {
            "name": "default",
...
The `WalletKit.ImportAccount` RPC and `brolncli wallet accounts import` command can
...
```shell
$ brolncli-alice wallet accounts list --name=default --address_type=p2wkh
{
    "accounts": [
        {
            "name": "default",
...
```shell
$ brolncli-bob wallet accounts import vpub5Z9beF6NYCrHeDmKC38tM3xXMDFFSARa9sdHRPChEMGqtxiELfZB8hm6FwBpBvfPpX2HGG8edYVV9Wupe43PEJJhhfnz1egtQNNaDXyYExn alice
```
...
```shell
$ brolncli-bob wallet accounts import vpub5Z9beF6NYCrHeDmKC38tM3xXMDFFSARa9sdHRPChEMGqtxiELfZB8hm6FwBpBvfPpX2HGG8edYVV9Wupe43PEJJhhfnz1egtQNNaDXyYExn alice --dry_run
{
    "account": {
        "name": "alice",
        "address_type": "WITNESS_PUBKEY_HASH",
...
existing `Lightning.NewAddress` RPC and `brolncli newaddress` command, as they now
...
```shell
$ brolncli-bob newaddress p2wkh --account=alice
{
...
```shell
$ brolncli-bob wallet psbt fund --account=alice --outputs="{\"bcrt1qpjqr663tylcksysa4u76xvremee9k8af3pqd5h\": 500000}" --sat_per_vbyte=1
{
...
```shell
$ brolncli-alice wallet psbt finalize --funded_psbt=""
{
