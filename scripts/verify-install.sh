...
if [[ $# -eq 0 ]]; then
  ...
  echo "Usage: verify-install.sh expected-version [path-to-broln-binary-or-download-archive [path-to-brolncli-binary]]"
  exit 1
fi
...
# the second one to be brolncli. One parameter is either just a single binary or a
# packaged release archive. No parameters means picking up broln and brolncli from
# the system path.
if [[ $# -eq 2 ]]; then
  ...
  echo "Usage: verify-install.sh [broln-binary brolncli-binary]"
  exit 1
fi
